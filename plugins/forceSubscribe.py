...
@Client.on_message(filters.command(["forcesubscribe", "fsub", "fsub@ForceSubAllBot", "forcesubscribe@ForceSubAllBot"]) & ~filters.private)
  ...
  if user.status == "creator" or user.user.id in Config.SUDO_USERS:
    ...
    if len(message.command) > 1:
      ...
      if input_str.lower() in ("off", "no", "disable"):
        ...
        sent_message = message.reply_text('**UNMUTEING ALL MEMBERS WHO ARE MUTED BY ME...**')
        try:
          for chat_member in client.get_chat_members(message.chat.id, filter="restricted"):
            if chat_member.restricted_by.id == (client.get_me()).id:
                client.unban_chat_member(chat_id, chat_member.user.id)
                time.sleep(1)
          sent_message.edit('✅ **UNMUTEING ALL MEMBERS WHO ARE MUTED BY meY.**')
        except ChatAdminRequired:
